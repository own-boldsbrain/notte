...
SNAPSHOT_DIR_TRAJECTORY: Final[Path] = DOM_REPORTS_DIR / Path(
    "trajectory_" + dt.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
)
...
@pytest.mark.parametrize("url, task", [(url, task) for url, task in test_cases()])
